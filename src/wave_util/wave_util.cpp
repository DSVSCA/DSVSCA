...
void WaveUtil::virtualize(const char * fileName, const char * output_fileName, const char * sofa_file, float x, float y, float z, Filter::Coord_Type coord_type, size_t block_size) {
    Wave * wave = readFile(fileName);
    ...
    if (coord_type == Filter::Spherical) mysofa_s2c(x_y_z);

    size_t data_length;
    bfloat ** audio = virtualize(wave, sofa_file, &data_length, x_y_z[0], x_y_z[1], x_y_z[2], block_size);
    if (audio == NULL) return;

    writeFile(output_fileName, wave, audio[0], audio[1], data_length);
...
float * WaveUtil::unpack_wave(bfloat * source, size_t source_length, size_t * output_size) {
...
WaveUtil::bfloat * WaveUtil::repack_wave(float * source, size_t source_length, uint32_t current_delay, uint32_t overall_delay, size_t * output_size) {
    const int16_t max = SHRT_MAX;
    const size_t samples_per_float = 2;
...
void WaveUtil::interleave(bfloat * left, bfloat * right, bfloat * out, const size_t length) {
    size_t out_index = 0;
...
WaveUtil::Wave * WaveUtil::readFile(const char * fileName) {
...
void WaveUtil::writeFile(const char * fileName, const Wave * wave, bfloat * left, bfloat * right, size_t length) {
...
WaveUtil::bfloat** WaveUtil::virtualize(const Wave * source, const char * sofaFile, size_t * data_length, float x, float y, float z, size_t block_size) {
    ...
    *data_length += (overall_delay / 2);
    ...
    bool left_success = left_conv.init(block_size, leftIR, filter_length);
    bool right_success = right_conv.init(block_size, rightIR, filter_length);
